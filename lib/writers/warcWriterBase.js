...
class WARCWriterBase extends EventEmitter {
    ...
    writeWarcInfoRecord(isPartOfV, warcInfoDescription, ua) {
        ...
        return this.writeRecordBlock(Buffer.concat([wh, whc, recordSeparatorBuffer], totalLength));
    }
    ...
    writeWarcMetadataOutlinks(targetURI, outlinks) {
        ...
        return this.writeRecordBlock(Buffer.concat([wmh, wmhc, recordSeparatorBuffer], totalLength));
    }
     ...
     * @return {Promise<void>}
     */
    writeRequestRecord(targetURI, httpHeaderString, requestData, uuids) {
        ...
        let reqWHeader = Buffer.from(
            warcRequestHeader({
                targetURI,
                concurrentTo: uuids['response'] ? uuids['response'] : null,
                now: this._now,
                rid: uuids['request'],
                len: reqHeadContentBuffer.length,
                digest:
                    process.env.NODEWARC_PAYLOAD_DIGEST && requestData
     ...
     * @return {Promise<void>}
     */
    writeResponseRecord(targetURI, httpHeaderString, responseData, uuid) {
        ...
        let respWHeader = Buffer.from(
            warcResponseHeader({
                ...
                digest:
                    process.env.NODEWARC_PAYLOAD_DIGEST && responseData
                        ? `sha1:${createHash('sha1')
                              .update(responseData)
                              .digest('hex')
                              .toUpperCase()}`
        ...
        const totalLength =
            respWHeader.length + resHeaderContentBuffer.length + CRLFBuffer.length + recordSeparatorBuffer.length;
        if (responseData !== null && responseData !== undefined) {
            ...
            return this.writeRecordBlock(
                Buffer.concat(
                    [respWHeader, resHeaderContentBuffer, responseDataBuffer, CRLFBuffer, recordSeparatorBuffer],
                    totalLength + responseDataBuffer.length
                )
            );
        } else {
            return this.writeRecordBlock(
                Buffer.concat([respWHeader, resHeaderContentBuffer, CRLFBuffer, recordSeparatorBuffer], totalLength)
            );
        }
    }
  ...
  writeRecordBlock (recordBuffer) {
    return new Promise((resolve, reject) => {
        // Check if we're in GZIPPED mode
        if (process.env.NODEWARC_WRITE_GZIPPED) {
            // Check if text/html shouldn't be zipped
            if (process.env.NODEWARC_DONT_ZIP_TEXT) {
                // If so then zip anything that isn't text/html
                if (!recordBuffer.includes('Content-Type: text/html')) {
                    recordBuffer = zlib.gzipSync(recordBuffer);
                }
            } else {
                // If everything should be zipped then zip away
                recordBuffer = zlib.gzipSync(recordBuffer);
            }
        }
