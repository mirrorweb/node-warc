...
class PuppeteerWARCGenerator extends WARCWriterBase {
   ...
   * @param {string | Buffer | null } [renderedContent]
   * @return {Promise<void>}
   */
  async generateWarcEntry(request, renderedContent) {
    ...
    if (response) {
      let resHTTP = `HTTP/1.1 ${response.status()} ${STATUS_CODES[response.status()]} ${CRLF}${stringifyHeaders(
        response.headers()
      )}`;
      let pageContent;
      let wasError = false;
      ...
      if (!wasError) {
        ...
        resHTTP = resHTTP.replace(replaceContentLen, `Content-Length: ${Buffer.byteLength(pageContent, 'utf8')}${CRLF}`);
      }
      await this.writeResponseRecord(this._UP.href, resHTTP, pageContent);
    }
  }
}
