 ...
 * @returns {string}
 */
function stringifyPuppeteerHeaders (headers) {
  ...
  while (i < len) {
    ...
    if (headKey === 'set-cookie') {
      // split on newlines
      const splitHeadValue = headValue.split('\n')
      // iterate through the cookies...
      for (const splitHeadValueItem of splitHeadValue) {
        // ...and write out the file.
        outString += `${startCase(headKey)
          .split(SPACE)
          .join(DASH)}: ${splitHeadValueItem}${CRLF}`
        i += 1
      }
    } else {
      outString += `${startCase(headKey)
